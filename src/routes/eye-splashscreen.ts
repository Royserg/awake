...
// Animation data
const ANIMATION_DATA: PixelLayerData[] = [
  {
    "id": "iytnzam0v",
    "name": "Layer 1",
    "keyframes": [
...
export class PixelAnimation implements PixelAnimationController {
  ...
  private readonly _layers: Map<string, { group: SVGGElement; data: PixelLayerData }> = new Map()
  ...
  private _render(): void {
    for (const [name, { group, data }] of this._layers) {
      ...
      const snapshots = this._getInterpolatedSnapshots(data.keyframes, this._progress)
  ...
  private _getInterpolatedSnapshots(
    ...
    const firstAppearances = this._getFirstAppearances(sorted)
    if (sorted.length === 1) {
      return sorted[0].objectSnapshots.filter(
    ...
    if (percent >= sorted[sorted.length - 1].percent) return sorted[sorted.length - 1].objectSnapshots
  ...
  private _buildLayers(): void {
    for (const layerData of ANIMATION_DATA) {
      ...
      if (layerData.keyframes.length > 0) {
        this._renderSnapshots(group, this._getInterpolatedSnapshots(layerData.keyframes, 0))
      }

      this.svg.appendChild(group)
      this._layers.set(layerData.name, { group, data: layerData })
    }
  }
